            ...
            { movie !== undefined ?
                <div className="MovieRoute">
                    ...
                    {moviesFavorited.find((movie) => movie.id === Number(id)) ? 
                        <button onClick={() => addMovieToFavList(movie)} value="add to favorites" /> :
                        <button onClick={() => removeMovieFromFavList(Number(id))} value="remove from favorites" />}
                    {moviesToWatch.find((movie) => movie.id === Number(id))  ? 
                        <button onClick={() => addMovieToWatchList(movie)} value= "add to watch list" /> :
                        <button onClick={() => removeMovieFromWatchList(Number(id))} value="remove from watch list" />} 
                </div> :
